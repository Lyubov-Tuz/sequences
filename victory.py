...
day_names = ['', 'первого', 'второго', 'третьего', 'четвертого', 'пятого', 'шестого', 'седьмого', 'восьмого',
    ...
    'двадцать седьмого', 'двадцать восьмого', 'двадцать деватого', 'тридцатого', 'тридцать первого']
...
while next_round == 'y':
    ...
    correct_answer_percent = (100 * correct_answer // (correct_answer + mistake))
    mistake_percent = (100 * mistake // (correct_answer + mistake))
    print(f'Верно угадано: {correct_answer} ({correct_answer_percent}%), ошибок: {mistake} ({mistake_percent}%)!\n')

    if correct_answer_percent == 100:
